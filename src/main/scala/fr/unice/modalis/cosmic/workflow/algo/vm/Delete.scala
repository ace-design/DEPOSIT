 ...
 */
case class DeleteElement[T<:DataType](val component: WFElement[T]) extends Instruction[T]{
   ...
   */
  override def make(w: Workflow[T]): Workflow[T] = w.deleteElement(component)

  override def toString:String = "{DeleteElement element=" + component.uid + "}"
}
 ...
 */
case class DeleteLink[T<:DataType](val component:WFLink[T]) extends Instruction[T]{
  /**
   * Apply the action on a workflow
   * @param w Workflow
   * @return A new workflow
   */
  override def make(w: Workflow[T]): Workflow[T] = w.deleteLink(component)
